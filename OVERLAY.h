...
namespace hijack
{
    ...
    __forceinline void hijack_init()
    {
        ...
        else
        {
            ...
            MessageBox(NULL, "overlay not found!", NULL, MB_OK);
            Shell_NotifyIcon(NIM_ADD, &nid);
            Shell_NotifyIcon(NIM_DELETE, &nid);
            exit(1);
        }
     ...
     //   LI_FN(ShowWindow)(DirectX9Interface::hijacked_hwnd, SW_SHOW);
        ...
        //DirectX9Interface::hijacked_hwnd = FindWindowA(E("CEF-OSC-WIDGET"), E("NVIDIA GeForce Overlay"));

        //while (!DirectX9Interface::hijacked_hwnd) {
        //    MessageBoxA(NULL, E("please open Nvidia"), E(""), NULL);
        //    DirectX9Interface::hijacked_hwnd = FindWindowA(E("CEF-OSC-WIDGET"), E("NVIDIA GeForce Overlay"));
        //}

        //// Set window transparency
        //SetWindowLong(DirectX9Interface::hijacked_hwnd, GWL_EXSTYLE,
        //    GetWindowLong(DirectX9Interface::hijacked_hwnd, GWL_EXSTYLE) | WS_EX_LAYERED | WS_EX_TRANSPARENT);

        //SetLayeredWindowAttributes(DirectX9Interface::hijacked_hwnd, RGB(0, 0, 0), 1000, LWA_ALPHA);

        //// Set the window to be topmost
        //SetWindowPos(DirectX9Interface::hijacked_hwnd, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);



        //MARGINS margins = { -1 };
        //if (DwmExtendFrameIntoClientArea(DirectX9Interface::hijacked_hwnd, &margins) != S_OK) {
        //    MessageBoxA(NULL, E("Failed to init overlay."), E("Code : 0x05"), NULL);
        //}

        //if (!SetWindowPos(DirectX9Interface::hijacked_hwnd, 0, 0, 0, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), SWP_SHOWWINDOW)) {
        //    MessageBoxA(NULL, E("Failed to init overlay."), E("Code : 0x05"), NULL);
        //}
    }
...
inline bool hide_window(uint32_t process_id, HWND window_id, bool hide)
{
    ...
    const uintptr_t user32_base = get_module_base(process_id, E("user32.dll"));
    if (!user32_base)
        return false;

    const uintptr_t function_pointer = get_module_export(process_handle, user32_base, E("SetWindowDisplayAffinity"));
    if (!function_pointer)
        return false;
...
void hide_overlay() {
    ...
    window = FindWindowA_Spoofed(E("CEF-OSC-WIDGET"), E("NVIDIA GeForce Overlay"));
