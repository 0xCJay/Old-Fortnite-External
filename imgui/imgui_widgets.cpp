...
bool ImGui::Renderingtab(const char* icon, bool selected)
{
    ...
    it_hover->second =  clamp(it_hover->second + (1.15f * GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 0.0f);
    ...
    ImColor Test;
    Test = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2], int(255 * it_filled->second));

    GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 4), ImColor(64, 64, 64, int(255 * GetStyle().Alpha)), icon);

    if (selected)
    {
      //  GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(21, 21, 21, int(255 * GetStyle().Alpha)), 5);
       // GetWindowDrawList()->AddRect(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Max.x, bb.Max.y), ImColor(41, 41, 41, int(255 * GetStyle().Alpha)), 5);

        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 5, bb.Min.y + 4), ImColor(183, 183, 183, int(255 * it_filled->second)), icon);
        GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x - 11 , bb.Min.y + 8.75f), ImVec2(bb.Min.x - 7 * it_filled->second, bb.Max.y - 8.75f), Test, 10.f, ImDrawCornerFlags_Right);
    }
...
bool ImGui::Renderingsubtab(const char* icon, bool selected)
{
    ...
    ImVec4 text = ImLerp(ImVec4{ 128 / 255.f, 128 / 255.f, 128 / 255.f, GetStyle().Alpha }, ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImVec4 text2 = ImLerp(ImVec4{ 128 / 255.f, 128 / 255.f, 128 / 255.f, GetStyle().Alpha }, ImVec4{ 255 / 255.f, 255 / 255.f, 255 / 255.f, GetStyle().Alpha }, it_filled->second);

    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 12, bb.Max.y - it_fill->second * 5), ImVec2(bb.Min.x + 35, bb.Max.y - 1), ImColor(255, 255, 255, int(255 * it_filled->second)), 3, ImDrawCornerFlags_Top);
...
bool ImGui::MenuTabEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y + size_arg.y - 2), ImVec2(pos.x + size_arg.x, pos.y + size_arg.y), ImColor(94, 159, 255, int(255 * it_active->second)));
    window->DrawList->AddRectFilledMultiColor(ImVec2(pos.x, pos.y + size_arg.y - 15), ImVec2(pos.x + size_arg.x, pos.y + size_arg.y), ImColor(94, 159, 255, 0), ImColor(94, 159, 255, 0),
        ImColor(94, 159, 255, int(100 * it_active->second)), ImColor(94, 159, 255, int(100 * it_active->second)));
...
bool ImGui::KeyBind(const char* label, KeyBind_t* k, ImGuiKeyBindFlags flags)
{
    ...
    ImColor Test;
    Test = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2], int(255 * it_hovered->second));
    ...
    window->DrawList->AddRect(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y), frame_bb.Max, ImColor(55, 54, 57, int(255 * fill)), style.FrameRounding, 15, 1.6f);
    window->DrawList->AddRect(ImVec2(frame_bb.Max.x - frame_w, frame_bb.Min.y), frame_bb.Max, ImColor(Test), style.FrameRounding, 15, 1.6f);
...
bool ImGui::MenuChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags, float rounding)
{
    ...
    parent_window->DrawList->AddRectFilled(p, p + size_arg, ImColor(16, 16, 17, 255), rounding);
    parent_window->DrawList->AddRect(p, p + size_arg, ImColor(41, 41, 41, 255), rounding, 0, 1);


    ImGui::PushFont(LexendLight);
    {
        if (!(flags & ImGuiWindowFlags_NoTitleBar))
            parent_window->DrawList->AddText(ImVec2(p.x + 8, p.y + 4), ImColor(150, 150, 150, int(255 * fill)), name);
    }
    ImGui::PopFont();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(15, 15, 15, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(45, 45, 45, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(Test), style.FrameRounding);
...
bool ImGui::CoolCheckbox(const char* label, bool* v)
{
    ...
    ImColor Checkboxcol;
    Checkboxcol = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2], t);
    window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 5), ImVec2(check_bb.Max.x - 5, check_bb.Max.y - 5), ImColor(35, 35, 35), 0);
    //box grab
   
  
    
       window->DrawList->AddRectFilled(ImVec2(check_bb.Min.x + 5, check_bb.Min.y + 5), ImVec2(check_bb.Max.x - 5, check_bb.Max.y - 5), Checkboxcol, 0);
...
void ImGui::Scrollbar(ImGuiAxis axis)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(94, 159, 255, int(100 * it_hovered->second)), style.ScrollbarRounding);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor(94, 159, 255, int(255 * it_active->second)), style.ScrollbarRounding);
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImColor(15, 15, 15, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(41, 41, 41, int(255 * fill)), style.FrameRounding, 15, 1.6f);
    ImColor Test;
    Test = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2],1);
    window->DrawList->AddRect(check_bb.Min, check_bb.Max, ImColor(Test), style.FrameRounding, 15, 1.6f);

    ImU32 check_col = GetColorU32(ImGuiCol_CheckMark);
    const float pad = ImMax(1.0f, (square_sz / 5.f));

    if(*v)
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), Test, square_sz - pad * 2.0f);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    ImColor Test;
    Test = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2], int(255 * fill));
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(15, 15, 15, 255), style.FrameRounding);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(53, 54, 57, 255), style.FrameRounding, 15, 1.6f);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, Test, style.FrameRounding, 15, 1.6f);

    ImGui::PushFont(LexendLight);
    {
        PushStyleColor(ImGuiCol_Text, ImColor(255, 255, 255, int(255 * fill)).operator ImVec4());
        RenderArrow(ImVec2(frame_bb.Max.x - arrow_size - style.FramePadding.x * 8, frame_bb.Min.y), popup_open ? ImGuiDir_Up : ImGuiDir_Down);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power)
{
    ...
    const ImRect rect(window->DC.CursorPos + ImVec2(5, 0), window->DC.CursorPos + ImVec2(size, 15.f));
    const ImRect clickable(window->DC.CursorPos + ImVec2(5.f, 17.f), window->DC.CursorPos + ImVec2(size, 15.f));
    ...
    // Slider behavior
    ImRect grab_bb;
    const bool value_changed = SliderBehavior(rect, id, data_type, v, v_min, v_max, format, power, ImGuiSliderFlags_None, &grab_bb);
    if (value_changed)
        MarkItemEdited(id);
    ...
    ImColor Test;
    Test = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2], int(255 * fill));
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 2.f), clickable.Max - ImVec2(0.f, 1.f), ImColor(35, 35, 35, 255), 0, 0);
    window->DrawList->AddRectFilled(clickable.Min + ImVec2(0.f, 2.f), grab_bb.Max - ImVec2(10.f, 1.f), ImColor(Test), 0, 0);
    window->DrawList->AddCircleFilled(grab_bb.Min + ImVec2(6.f, 13), 7 + it_anim->second.circle_anim, ImColor(255, 255, 255, 255), 70.f);
    window->DrawList->AddCircle(grab_bb.Min + ImVec2(6.f, 13), 7 + it_anim->second.circle_anim, ImColor(Test), 100.f);
    ...
    ImGui::PushFont(LexendLight);
    {
        ...
        RenderText(ImVec2(rect.Max.x - label_buffer_size.x, rect.Min.y - 8), value_buf, value_buf_end);

        if (label_size.x > 0.0f)
            RenderText(ImVec2(rect.Min.x, rect.Min.y - 8), label);
        PopStyleColor();
    }
    ImGui::PopFont();
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (!is_multiline)
    {
        ...
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(45, 45, 45, int(255 * fill)), style.FrameRounding);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(94, 159, 255, int(255 * it_hovered->second)), style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            PushStyleColor(ImGuiCol_Text, ImColor(75, 75, 75, int(255 * fill)).operator ImVec4());
            RenderTextClipped(ImVec2(frame_bb.Min.x + style.FramePadding.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max - style.FramePadding, buf_display, buf_display_end, NULL, ImVec2(0.0f, 0.5f));
        ...
        if (render_cursor)
        {
            ...
            ImVec2 cursor_screen_pos = draw_pos + cursor_offset - draw_scroll + ImVec2(5, 4);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    ImGui::PushFont(LexendLight);
    {
        ...
        if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
        {
            ...
            SetCursorPosX(GetWindowSize().x - style.WindowPadding.x - square_sz - 5);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(94, 159, 255, int(255 * it_hovered->second)), rounding);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImColor Test;
    Test = ImVec4(accentcolor[0], accentcolor[1], accentcolor[2], int(255 * it_active->second));
    // Render
    window->DrawList->AddRectFilled(bb.Min, ImVec2(bb.Min.x + 4 * it_active->second, bb.Max.y), Test, 10.f, ImDrawCornerFlags_Right);
    ...
    {
        PushStyleColor(ImGuiCol_Text, ImColor(75, 75, 75, int(255 * fill)).operator ImVec4());
        RenderTextClipped(ImVec2(bb_inner.Min.x + style.WindowPadding.x, bb_inner.Min.y - 3), bb_inner.Max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
