...
public:
	vector3 get_bone_3d(int id) const {
		...
		uintptr_t bone_array = request->read<uintptr_t>((u)this + 0x5B0);
		if (bone_array == NULL) bone_array = request->read<uintptr_t>((u)this + 0x5B0 + 0x10);
	...
	//	int is_cached = request->read<int>((u)this + 0x5D0);
	//	auto bone_transform = request->read<FTransform>(request->read<u>((u)this + 0x10 * is_cached + 0x5B0) + 0x60 * id);
...
bool populate_global_data(const std::shared_ptr<global_s>& global_temp) {
	...
	SPOOF_FUNC
		...
		global_temp->world = request->read<uworld*>(Bypass::GameBase + 0x126CF528);
...
bool populate_player_data(const std::shared_ptr<global_s>& global_temp,
	...
	{
		auto is_despawning = (request->read<char>((u)player_data_temp->player + 0x758) >> 3);
		if (is_despawning) return false;
	}
